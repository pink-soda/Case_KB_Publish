...
class CategoryHierarchyBuilder:
    ...
    def process_case(self, email_content: str, file_path: str = None) -> dict:
        ...
        try:
            ...
            # 添加调试日志
            logger.debug(f"原始 LLM 响应: {result}")
            logger.debug(f"处理后的结果: {processed_result}")
    ...
    def fix_file_paths(self):
        """修复已存在的分类结果中的文件路径格式"""
        for case in self.classified_cases:
            if 'file_path' in case:
                # 统一使用正斜杠
                path = case['file_path'].replace('\\', '/')
                # 确保路径以 ./ 开头
    ...
    def _save_case(self, processed_result: dict, content: str, file_path: str = None):
        """保存分类结果到案例列表中"""
        case_data = {
            'content': content,
            'file_path': file_path,
            'classification': {
                'level1': processed_result['level1'],
                'level2': processed_result['level2'],
                'level3': processed_result['level3']
            },
            'category_score': {
                'level1': processed_result['similarity']['level1'],
                'level2': processed_result['similarity']['level2'],
                'level3': processed_result['similarity']['level3']
            },
        ...
        if file_path:
            for case in self.classified_cases:
                if case.get('file_path') == file_path:
                    case.update(case_data)
                    break
            else:  # 如果没找到现有案例，添加新案例
        ...
        try:
            ...
            logger.error(f"保存案例到文件时出错: {str(e)}")
            raise

    def _update_hierarchy(self, classification_result: dict):
        """更新分类层级结构"""
        level1 = classification_result['level1']
        level2 = classification_result['level2']
        level3 = classification_result['level3']
        ...
        if level1 not in special_categories:
            if level1 not in self.hierarchy:
                self.hierarchy[level1] = {}
            
            if level2 not in special_categories:
                if level2 not in self.hierarchy[level1]:
                    self.hierarchy[level1][level2] = []
                
                if level3 not in special_categories and level3 not in self.hierarchy[level1][level2]:
                    self.hierarchy[level1][level2].append(level3) 