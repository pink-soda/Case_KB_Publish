...
class CategoryHierarchyBuilder:
    def __init__(self, llm_handler, similarity_threshold=0.8, initial_hierarchy=None, initial_cases=None):
        ...
        {{
            ...
            "level2": "二级分类名称",
            "level3": "三级分类名称",
            "similarity_level1": 0.0,  # 与现有一级分类的相似度（0-1），如果是新分类则为0
            "similarity_level2": 0.0,  # 与现二级分类的相似度（0-1），如果是新分类则为0
            "similarity_level3": 0.0,  # 与现三级分类的相似度（0-1），如果是新分类则为0
            "matched_path_level1": "",  # 匹配到的现有一级分类路径，如果是新分类则为空
            "matched_path_level2": "",  # 匹配到的现有二级分类路径，如果是新分类则为空
            "matched_path_level3": "",  # 匹配到的现有三级分类路径，如果是新分类则为空
            "reasoning": "分类理由说明"
        }}
        """
